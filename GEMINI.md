...
- For mass clearing or replacement of formatting (e.g. highlighting colors), prefer bulk methods like `Range.Replace` with `SearchFormat:=True` and `ReplaceFormat:=True` configured via `Application.FindFormat`/`Application.ReplaceFormat` rather than cell-by-cell scan-and-clear loops.
- Optimize bulk cell transformation helpers (such as `ICellTransformer`) by using lazy format evaluation flags (e.g. `__MIXED__` and `__FORCE_GENERAL_IF_TEXT__`) to defer reading cell-by-cell formatting properties when selection formatting is mixed.
